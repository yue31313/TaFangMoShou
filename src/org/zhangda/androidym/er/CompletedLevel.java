...
{
	public static void PersistLevelCompleted(MainGame game, Difficulty difficulty, int level)
	{
		boolean flag = false;
		int numRemainingLives = game.getGameplayScreen().getRemainingLives().getNumRemainingLives();
		for (CompletedLevel level2 : game.getCompletedLevels())
		{
			if ((level2.getDifficulty() == difficulty.getValue()) && (level2.getLevel() == level))
			{
				flag = true;
